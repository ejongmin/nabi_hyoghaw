...
def compute_incremental_zscores(new_agg: pd.DataFrame, existing: pd.DataFrame,
    ...
    for cid in companies:
        ...
        has_hist_fb2 = len(hist_fb2) >= 12
        mu_fb2   = hist_fb2.mean() if has_hist_fb2 else np.nan
        sig_fb2  = hist_fb2.std(ddof=1) if has_hist_fb2 else np.nan

        hist_sup = ex_cid.loc[ex_cid["sup_n_fb2"].fillna(0) >= sup_strict, "sup_fb2_mean"].dropna()
        has_hist_sup = len(hist_sup) >= 6
        mu_sup   = hist_sup.mean() if has_hist_sup else np.nan
        sig_sup  = hist_sup.std(ddof=1) if has_hist_sup else np.nan
        ...
        for _, row in new_cid.iterrows():
            ...
            ym_str       = str(row["ym"])
            # historical 통계가 충분해야만 reliable (rebuild_zscore_v3.py와 동일 기준)
            reliable     = (n_fb2 >= fb2_strict) and has_hist_fb2

            if reliable and not np.isnan(fb2_mean) and not np.isnan(sig_fb2) and sig_fb2 > 1e-10:
                z = float(np.clip((fb2_mean - mu_fb2) / sig_fb2, -5.0, 5.0))
            else:
                z = np.nan
            z_source = "finbert2" if reliable and not np.isnan(z) else "insufficient"

            if (sup_n_fb2 >= sup_strict and has_hist_sup
                    and not np.isnan(sup_fb2_mean)
                    and not np.isnan(sig_sup) and sig_sup > 1e-10):
                sz = float(np.clip((sup_fb2_mean - mu_sup) / sig_sup, -5.0, 5.0))
            else:
                sz = np.nan
