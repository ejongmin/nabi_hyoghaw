"""
Phase 4 DID (Difference-in-Differences) 분석 — 44개 기업 버전
================================================================
...
변경점 (44개 기업 버전):
  - PATH_TONE  → tone_all44_expanding_compat.parquet
  - PATH_EDGES → experiments/universe_44/data/seed/seed_edges_44.csv
  - PATH_C44   → experiments/universe_44/data/universe/companies_44.csv
  - PATH_MKTMAP→ experiments/universe_44/configs/market_proxy_mapping_44.yaml
  - OUT_RPT    → reports/phase4_did_44.txt
  - OUT_CSV    → reports/phase4_did_44_detail.csv
"""
...
PATH_TONE  = ROOT/"data/processed/tone_all44_expanding_compat.parquet"
PATH_PRICES= ROOT/"data/processed/prices_daily.parquet"
PATH_MKT   = ROOT/"data/processed/market_proxies.parquet"
PATH_MKTMAP= ROOT/"experiments/universe_44/configs/market_proxy_mapping_44.yaml"
PATH_EDGES = ROOT/"experiments/universe_44/data/seed/seed_edges_44.csv"
PATH_C44   = ROOT/"experiments/universe_44/data/universe/companies_44.csv"
OUT_RPT    = ROOT/"reports/phase4_did_44.txt"
OUT_CSV    = ROOT/"reports/phase4_did_44_detail.csv"
...
def main():
    log.info("Phase 4 DID 분석 시작 (44개 기업 버전)")
    tone = pd.read_parquet(PATH_TONE)
    c44  = pd.read_csv(PATH_C44)
    nm   = dict(zip(c44["company_id"], c44["canonical_name"]))
    stage= dict(zip(c44["company_id"], c44["stage_bucket"]))
    cids = c44["company_id"].tolist()
    log.info("대상 기업 수: %d", len(cids))

    cd_map   = build_company_data(cids)
    log.info("가격 데이터 로드 완료: %d개 기업", len(cd_map))
    conn_map = build_supply_adjacency(PATH_EDGES, cids)
    log.info("공급망 연결 맵 구축 완료: %d개 충격 기업에 연결 존재", len(conn_map))
    ...
    # ── DID 분석 ──
    SEP = "="*68
    lines = [SEP, "Phase 4 DID 분석 (44개 기업) — 공급망 효과 vs 섹터 효과 분리", SEP, "",
             "설계:",
    ...
    log.info("리포트 저장: %s", OUT_RPT)
    log.info("상세 CSV 저장: %s", OUT_CSV)
    print("\n" + rpt)
