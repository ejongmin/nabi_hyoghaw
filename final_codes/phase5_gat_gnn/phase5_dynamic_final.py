...
COMMODITY_COLS = [
    "nickel_z",     # 니켈: VALE (FRED PNIUSDM → yfinance VALE)
    "cobalt_z",     # 코발트: GLNCY (FRED PCOBALTDM → yfinance GLNCY)
    "aluminum_z",   # 알루미늄: ALI=F (FRED PALUMUSDM → yfinance COMEX선물)
    "copper_z",     # 구리: HG=F (FRED PCOPPUSDM → yfinance COMEX선물)
    "lithium_z",    # 리튬: ALB (FRED 미존재 → yfinance Albemarle)
]
FEAT_COLS = FEAT_COLS + COMMODITY_COLS   # 13 + 5 = 18
N_FEAT = len(FEAT_COLS)                  # 18
...
def build_feature_matrix(tone_path, ar_df, cids):
    ...
    # ── commodity 데이터 로드 및 병합 ────────────────────────────────────
    # tone 단계에서 COMMODITY_COLS는 missing으로 0 초기화됨.
    # 실제 데이터가 있으면 여기서 덮어씀 (컬럼 충돌 방지: drop 후 merge).
    # LME 실제 원자재 가격 우선, 없으면 ETF 기반으로 폴백
    PATH_COMM = ROOT / "data/processed/commodity_lme_zscore.parquet"
    if not PATH_COMM.exists():
        log.warning("LME 가격 파일 없음 → ETF 기반 폴백: commodity_monthly_zscore.parquet")
        PATH_COMM = ROOT / "data/processed/commodity_monthly_zscore.parquet"
    if PATH_COMM.exists():
        comm = pd.read_parquet(PATH_COMM)
        comm["ym"] = pd.to_datetime(comm["year_month"]).dt.to_period("M")
        # 기존 0-초기화 컬럼 제거 후 실제 값으로 대체
        panel = panel.drop(columns=COMMODITY_COLS, errors="ignore")
        panel = panel.merge(comm[["ym"] + COMMODITY_COLS], on="ym", how="left")
        # 결측 → 0 채우기 (z-score이므로 0=중립)
        for c in COMMODITY_COLS:
            panel[c] = panel[c].fillna(0.0)
        log.info("commodity 피처 병합 완료: %s", COMMODITY_COLS)
    else:
        log.warning("commodity 데이터 없음 (%s) - 해당 피처 0으로 대체", PATH_COMM)
        for c in COMMODITY_COLS:
            panel[c] = 0.0
    ...
    # 이상값 방지: z-score 계열 피처 클리핑 (|z|>5 → ±5)
    for c in Z_COLS:
